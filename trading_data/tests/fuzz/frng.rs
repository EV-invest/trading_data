...
//! no draw ever panics.
//!
//! Ported verbatim from `dockviewers_core/tests/integration/frng.rs`; see
//! [`README.md`](README.md) on why this is not `rand` and not `proptest`.
...
impl Frng {
	...
	/// tiny `n` (lane counts, action kinds), so single-byte modulo bias is irrelevant.
	...
	/// byte, so it silently collapses for `n > 256` — an arrival gap spans nanoseconds and would
	/// otherwise never leave the first cell.
