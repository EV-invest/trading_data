...
pub fn node(attr: TokenStream, item: TokenStream) -> syn::Result<TokenStream> {
	...
	let (kernel, emit) = match trait_name.as_str() {
		...
		"Emit" => {
			return Err(syn::Error::new_spanned(
				path,
				"`impl Emit` is written by `#[node]`, not by hand: a run-shaped node declares which kernel fills it. Write `impl Scans` for a per-element `Expr` body, `impl Closes` for one that accumulates whole periods, `impl Folds` for a recurrence, or `impl Runs` — the stated hatch, which needs a `const WHY`",
			));
		}
		_ =>
			return Err(syn::Error::new_spanned(
