...
/// leg meant to go dark with it name it in [`Node::When`]. [`Horizon::Unbounded`] by construction —
/// a latched bit is exactly the state nothing reconstitutes — which is also why it can never itself
/// be gated.
///
...
/// consumers, and all of them sit behind one common **non-latch** gate (other than `name` itself) —
/// sampling it while that gate is closed is pure waste, so it must be gated too or declare
