	...
	fn live(&self) -> Vec<bool> {
		...
		ns.iter()
			.map(|n| {
				...
				// nothing to re-derive: a node a latch may not darken was given `⊤` by the demand rule
				// itself, and reads back here as the empty conjunct.
				let demanded = n.demand.iter().any(|conj| conj.iter().all(|&g| self.open(ns[g].name)));
				demanded && own_open
			})
			.collect()
	}
