//! Low-level trade primitives shared by the exchange bridge (`v_exchanges`) and the persistence
//! tier. Both sides see the same [`BatchTrades`], so a websocket stream parses straight into
//! persistence rows with no lossy hand-off — this crate is that shared parse boundary.

use jiff::Timestamp;
use v_utils::trades::{PrecisionPriceQty, Side, Timestamped};
