...
use trading_data::{Asset, Catalog, Feather, Mc, Precision, PrecisionPriceQty, Row as _, Ts};
...
fn main() {
	...
	let s = Situation {
		pair: "TAO-USDT".into(),
		bybit_symbol: "TAOUSDT".into(),
		coingecko_id: "bittensor".into(),
		precision: PrecisionPriceQty::new(Precision(2), Precision(3)),
	...
	let t = Instant::now();
	let catalog = ensure_lanes(&cache, &s);
